...
async function run() {
  ...
  for (const sourcePath of relevantFiles) {
    ...
    for (const variant of variants) {
      const targetExt = ext === '.heic' ? '.jpg' : ext;
      const fileName = `dama-venus-${bereich}-${motiv}-${toPositionalIndex(nextIndex)}-${variant}${targetExt}`;
        ...
        if (ext === '.heic') {
          if (!detectedHeicTool) {
            ...
            itemResult.variants.push({
              ...
              skipped: true,
              reason: 'HEIC-Tooling nicht verfügbar',
              fallback: 'non-blocking-skip',
            });
            continue;
          }

          const processed = await processHeicVariant(sourcePath, targetPath, detectedHeicTool);
          if (!processed) {
            counters.errors += 1;
            assetHasError = true;
